...
import {samyakService} from '../redux/service/OauthService';
import {setTokenDetails} from '../redux/slice/OauthSlice';
...
const baseQuery = fetchBaseQuery({
  ...
  prepareHeaders: (headers, {getState}) => {
    ...
    const token = (getState() as RootState).samyak.tokenDetails;
...
> = async (args, api, extraOptions) => {
  ...
  if (result.error && result.error.status === 401) {
    ...
    const refresh_token = store.getState().samyak.tokenDetails?.refresh_token;
